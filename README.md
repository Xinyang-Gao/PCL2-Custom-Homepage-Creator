...
- **属性面板编辑**  
  按“内容”“外观”“布局”“行为”分组显示组件属性，支持 `Margin` 四边独立编辑、枚举下拉、自定义属性。

- **网格布局可视化编辑器**  
  以表格形式编辑 `Grid` 的列/行定义（像素、星号、自动），支持最小/最大宽度约束。
...
- **事件绑定配置**  
  为按钮/列表项配置 `EventType`（打开网页、启动游戏、弹出窗口等）与 `EventData`，为后续运行时交互预留数据。
...
- **编辑属性**：修改属性后自动生效（部分输入框防抖）。
- **删除/复制**：选中组件后点击属性面板底部按钮，或使用快捷键（见后）。
